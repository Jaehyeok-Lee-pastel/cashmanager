...
gemini -m gemini-3-flash-preview -p "Research the library 'httpx' comprehensively.
...
gemini -m gemini-3-flash-preview -p "Research 'pydantic' v2 with focus on:
...
1. **Run Gemini research** (background)
   ```bash
   gemini -m gemini-3-flash-preview -p "Research: {library}" 2>/dev/null
   ```
