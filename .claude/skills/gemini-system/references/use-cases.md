...
```bash
# General research
gemini -m gemini-3-flash-preview -p "Research best practices for implementing OAuth2 in Python.
Include:
...
gemini -m gemini-3-flash-preview -p "Research FastAPI authentication patterns in 2025.
Focus on:
...
gemini -m gemini-3-flash-preview -p "Analyze this entire codebase and provide:
...
gemini -m gemini-3-flash-preview -p "Trace the data flow for user authentication:
...
gemini -m gemini-3-flash-preview -p "Analyze this tutorial video:
...
# Code review video
gemini -m gemini-3-flash-preview -p "Extract code patterns and best practices demonstrated in this video" < code-review.mp4 2>/dev/null
```
...
```bash
# Meeting recording
gemini -m gemini-3-flash-preview -p "Transcribe and summarize this technical discussion:
- Key decisions made
- Action items
- Open questions
...
gemini -m gemini-3-flash-preview -p "Extract technical insights from this talk about {topic}" < conference-talk.mp3 2>/dev/null
```
...
```bash
# API documentation
gemini -m gemini-3-flash-preview -p "Extract from this API documentation:
...
gemini -m gemini-3-flash-preview -p "Summarize this technical specification:
- Core requirements
- Constraints
...
# Research paper
gemini -m gemini-3-flash-preview -p "Analyze this paper and explain:
...
gemini -m gemini-3-flash-preview -p "Find and summarize the latest React 19 features and migration guide from official docs" 2>/dev/null

# Compare libraries
gemini -m gemini-3-flash-preview -p "Compare these Python HTTP clients in 2025:
...
# Troubleshooting
gemini -m gemini-3-flash-preview -p "Research common causes and solutions for: {error message}
...
```bash
# Framework migration
gemini -m gemini-3-flash-preview -p "Analyze our codebase for Django to FastAPI migration:
...
# Version upgrade
gemini -m gemini-3-flash-preview -p "Research breaking changes from Python 3.11 to 3.13.
...
```bash
gemini -m gemini-3-flash-preview -p "List all API endpoints in this codebase with their HTTP methods" \
...
```bash
gemini -m gemini-3-flash-preview -p "Generate comprehensive documentation for src/auth/" \
