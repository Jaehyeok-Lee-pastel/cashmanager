...
**Gemini CLI (gemini-3-flash-preview) is your research specialist with 1M token context.**

> ⚠️ **2026-05-27**: 모델을 `-m`으로 명시하라(Pro 풀 소진 — `-m` 없으면 실패). 기본 `gemini-3-flash-preview` → 실패 시 `gemini-3.1-flash-lite-preview` → `codex`(3단 폴백). 구 Gemini CLI는 2026-06-18 종료(후속 agy는 headless 캡처 버그로 자동화 불가). `--output-format json`은 미지원. 상세: `.claude/rules/gemini-delegation.md`
...
- prompt: |
    ...
    gemini -m gemini-3-flash-preview -p "{research question}" 2>/dev/null
...
```bash
gemini -m gemini-3-flash-preview -p "Brief question" 2>/dev/null
```
...
```bash
# Codebase analysis
gemini -m gemini-3-flash-preview -p "{question}" --include-directories . 2>/dev/null

# Multimodal (PDF/video/audio)
gemini -m gemini-3-flash-preview -p "{prompt}" < /path/to/file.pdf 2>/dev/null

# JSON output
gemini -m gemini-3-flash-preview -p "{question}" --output-format json 2>/dev/null
```
...
```bash
gemini -m gemini-3-flash-preview -p "Research best practices for {feature} in Python 2025.
Include:
...
```bash
gemini -m gemini-3-flash-preview -p "Analyze this repository:
...
```bash
# Video
gemini -m gemini-3-flash-preview -p "Analyze video: main concepts, key points, timestamps" < tutorial.mp4 2>/dev/null

# PDF
gemini -m gemini-3-flash-preview -p "Extract: API specs, examples, constraints" < api-docs.pdf 2>/dev/null

# Audio
gemini -m gemini-3-flash-preview -p "Transcribe and summarize: decisions, action items" < meeting.mp3 2>/dev/null
```
