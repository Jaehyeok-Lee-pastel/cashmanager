...
def extract_codex_prompt(command: str) -> str | None:
    ...
    patterns = [
        # Prompt is the first quoted string after `codex exec`; flags are unquoted.
        r'codex\s+exec\s+.*?"([^"]+)"',
        r"codex\s+exec\s+.*?'([^']+)'",
    ]
...
def extract_gemini_prompt(command: str) -> str | None:
    ...
    patterns = [
        # allow flags (e.g. -m gemini-3-flash-preview) between `gemini` and `-p`
        r'gemini\s+.*?-p\s+"([^"]+)"',
        r"gemini\s+.*?-p\s+'([^']+)'",
    ]
    for pattern in patterns:
...
def main() -> None:
    ...
    # Output notification using standard PostToolUse hookSpecificOutput format
    print(
        json.dumps(
            {
                "hookSpecificOutput": {
