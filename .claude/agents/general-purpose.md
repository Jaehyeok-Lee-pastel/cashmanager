...
```bash
# Analysis (read-only)  — --skip-git-repo-check: 이 워크스페이스는 git 레포가 아님
codex exec --sandbox read-only --skip-git-repo-check "{question}" 2>/dev/null

# Implementation — ISOLATED files ONLY (new files / temp paths Claude is NOT tracking).
# ⚠️ two-writer desync: live/shared/rule-bound files → have Codex RETURN a diff, Claude applies it.
# After this call, Claude MUST Read() the affected files before any further Edit/Write.
codex exec --sandbox workspace-write --skip-git-repo-check "{task}" 2>/dev/null
```
