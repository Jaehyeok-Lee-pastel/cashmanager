...
async def list_budgets(user: CurrentUserDep) -> list[BudgetOut]:
    return budget_service.list_budgets(user.user_id)


@router.put("", response_model=list[BudgetOut])
async def upsert_budgets(payload: BudgetUpsert, user: CurrentUserDep) -> list[BudgetOut]:
...
async def suggest_budgets(user: CurrentUserDep) -> list[BudgetSuggestion]:
    return budget_service.suggest_budgets(user.user_id)


@router.delete("/{category_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_budget(category_id: str, user: CurrentUserDep) -> None:
